...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function loadAndInitCanvas(canvasID) {
        ...
        for(let i = 0; i < frameCount; i++){
            ...
            img.onload = function() {
                imagesLoaded++;
                if (imagesLoaded === frameCount) initCanvas(canvasID, images);
            };
            img.onerror = function() {
    ...
    function currentFrame(index, canvasID) {
        ...
        const frameUrl = `${baseUrl}/MOBI_${canvasID === "footer_canvas" ? "Footer" : "RollingBall_viewport"}_${screenWidth < breakpoint ? "9-16" : "16-9"}_${index.toString().padStart(5, "0")}.jpg`;
        return frameUrl;
    }
    function initCanvas(canvasID, images) {
        const canvas = document.querySelector(`#${canvasID}`);
        const context = canvas.getContext("2d");
        const triggerElement = canvasID === "footer_canvas" ? document.querySelector(".scrub-wrapper-footer") : document.querySelector(".scrub-wrapper");
        const frameCount = 200;
        const airpods = {
            frame: 0
        };
